...
# unicode symbols 统一码符号
COPYRIGHT, PHON_COPYRIGHT = "\u2117", "\u00a9"
# if a metadata block exceeds this, mutagen will raise error   如果元数据块超过此值
# and the file won't be tagged   并且该文件不会被标记
...
def _embed_flac_img(root_dir, audio: FLAC):
    ...
    try:
        # rest of the metadata still gets embedded  其余元数据仍然被嵌入
        # when the image size is too big  当图像尺寸太大时
...
# Use KeyError catching instead of dict.get to avoid empty tags 使用 KeyError 捕获代替 dict.get 以避免空标签
def tag_flac(
    ...
    if "Disc " in final_name:
        audio["DISCNUMBER"] = str(d["media_number"])  #媒体编号

    try:
        audio["COMPOSER"] = d["composer"]["name"]  # COMPOSER 作曲家
    except KeyError:
        pass

    artist_ = d.get("performer", {}).get("name")  # TRACK ARTIST 曲目艺术家
    if istrack:
    ...
    audio.save()
    os.rename(filename, final_name)    ##抓取web 
    os.system(f"flac -f8 \"{final_name}\"")  ##抓取web后执行flac -f8压缩转码
def tag_mp3(filename, root_dir, final_name, d, album, istrack=True, em_image=False):
    ...
    # temporarily holds metadata 暂时保存元数据
    tags = dict()
    ...
    # write metadata in `tags` to file  将“tags”中的元数据写入文件
