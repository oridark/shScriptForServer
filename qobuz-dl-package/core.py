    ...
    def download_from_id(self, item_id, album=True, alt_path=None):
        ...
        try:
            ...
            if self.quality != 5:
                ...
                #     try:
                #         cur.execute("""delete from albums where qobuzId=?;""", (item_id,))
                #         cur.execute("""delete from redAlbums where qobuzId=?;""", (item_id,))
                #         # if (self.quality == 27 or self.quality == '27'):
                #         #     print('self.quality:', 27)
                #         #     cur.execute("""delete from albums where qobuzId=?;""", (item_id,))
                #         # elif(self.quality == 6 or self.quality == '6'):
                #         #     print('self.quality:', 6)
                #         #     cur.execute("""UPDATE albums SET downloadStatusFor16bit=?, fileDir=? where qobuzId=?;""", ("2", '', item_id,))
                #         conn.commit()
